...
class Advertise extends MY_Controller
{
    ...
    {
        $data=$this->My_model->getresult("SELECT * from advertise LIMIT 0,1");
        $this->data['data']=$data[0];
        $seo_data=$this->My_model->getresult("SELECT * from seo_individual WHERE  individual='AD' LIMIT 0,1");
        $this->data['seo_data'] = $seo_data[0];
        $this->data['message'] = $this->session->flashdata("message");
        $this->data['view_name'] = "advertise_view.php";
    ...
    {
        $this->data['message'] = $this->session->flashdata("message");
        $this->data['view_name'] = "advertise_form_view.php";
        $this->load->view('backend/layout', $this->data);
    }
    ...
    public function save()
    {
        ...
        if(!empty($id)) {
            $this->My_model->updatedata("advertise",$data,array("id"=>$id));
        }else{
            $this->My_model->insertdata($data, "advertise");
        }

        $this->session->set_flashdata("message","success_Advertise Content Save Successfully.");
        redirect("backend/advertise");
    ...
    public function form_list()
    {
        ...
        $totalData = $this->My_model->getresult("select count(id) as tot from advertise_data");
        ...
        if(empty($this->input->post('search')['value']) && empty($year) && empty($month) && empty($search_keyword))
        {
            $posts = $this->My_model->getresult("
               ...
               SELECT ".implode(',',$columns)." FROM advertise_data
               ORDER BY $order $dir
               LIMIT $start,$limit
            ...
            $totalData = $this->My_model->getresult("select count(id) as tot from advertise_data $where");
            $totalFiltered = $totalData[0]['tot'];
        }
