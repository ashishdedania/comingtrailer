 ...
 * @author yoo
 */
class MoviePoster extends CI_Controller {
    ...
    public function index($cat_id = 3, $subcat_id = 0) {
        ...
        $this->data['cat'] = $this->adm_home_model->getSubCat(3);
        $this->data['cat_id'] = 3;
        ...
        $this->data['poster'] = $this->WebService->getAllPoster($subcat_id, 50, 0);
...
//        }
        ...
        $links = 'MoviePoster/index/' . $cat_id . '/' . $subcat_id;
        if ($subcat_id == 0) {
        ...
        $this->load->view('movie_poster', $this->data);
...
//        $_POST['search_keyword'] = $this->input->post('cat_id');
        $this->data['poster'] = $this->WebService->getAllPoster($subcat_id, 50, $start);
        $this->load->view('poster_pagination', $this->data);
    }
}
