 ...
 * @author yoo
 */
class AdmViewSinger extends MY_Controller{
    //put your code here
    ...
    public function index(){
        ...
        $query = $this->db->get_where('seo_individual',array('individual' => 'S')); 
        $this->data['seo'] = $query->row_array();
        $this->data['year_list'] = $this->WebService->getMinYear('singer','created');
        $this->load->view('header_footer/header');
        $this->load->view('header_footer/sidebar');
        $this->load->view('adm_view_singer',$this->data);
        $this->load->view('header_footer/footer');
        
    }
    
    public function getAllActors($status = 0){
        return $this->adm_data_model->getAllData('singer','singer_id','singer_name',$status);
    }
    ...
    public function edit($id,$mstp='m'){
        ...
        $this->data['actors'] = $this->adm_data_model->getRowData('singer',$id);
        $this->data['mstp'] = $mstp;
        $this->data['actors_data'] = '';
        foreach($individual_map as $key => $val){
            $individual_mstp_data = json_decode($this->WebService->getIndividualData($id,'singer',$val));
            if($val == $mstp){
        ...
        $this->load->view('header_footer/header');
        $this->load->view('header_footer/sidebar');
        $this->load->view('adm_edit_singer',$this->data);
    }
