 ...
 * @author yoo
 */
class AdmViewDirector extends MY_Controller{
    //put your code here
    ...
    public function index(){
        ...
        $query = $this->db->get_where('seo_individual',array('individual' => 'D')); 
        $this->data['seo'] = $query->row_array();
        $this->data['year_list'] = $this->WebService->getMinYear('director','created');
        $this->load->view('header_footer/header');
        $this->load->view('header_footer/sidebar');
        $this->load->view('adm_view_director',$this->data);
        $this->load->view('header_footer/footer');
        
    }
    
    public function getAllActors($status = 0){
        return $this->adm_data_model->getAllData('director','director_id','director_name',$status);
    }
    ...
    public function edit($id,$mstp='m'){
        ...
        $total_mstp = array();
        $this->data['actors'] = $this->adm_data_model->getRowData('director',$id);
        $this->data['mstp'] = $mstp;
        $this->data['actors_data'] = '';
        foreach($individual_map as $key => $val){
            $individual_mstp_data = json_decode($this->WebService->getIndividualData($id,'director',$val));
            if($val == $mstp){
        ...
        $this->load->view('header_footer/header');
        $this->load->view('header_footer/sidebar');
        $this->load->view('adm_edit_director',$this->data);
    }
