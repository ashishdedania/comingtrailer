<div class="ctbody-content">
    ...
    <div class="user-profile cf">
        <?php echo form_open_multipart((!empty($edit_data)) ? base_url('backend/singer/update') : base_url('backend/singer/save'), array('method' => 'POST')); ?>
        <?php if (!empty($edit_data)) { ?>
            <input type="hidden" name="id" value="<?php echo $edit_data['id']; ?>">
<?php } ?>
        <div class="image-name">
            <img src="<?php echo (!empty($edit_data) && !empty($edit_data['singer_img'])) ? base_url() . "images/singers/" . $edit_data['singer_img'] : base_url() . 'images/no-user.svg'; ?>" alt=""  id="img">
        ...
        <div class="info-block">
            <div class="input-wrap">
                <label>Name</label>
                <div class="cf"><input type="text" name="name" value="<?php if (!empty($edit_data)) {
    echo $edit_data['singer_name'];
} ?>" required="" class="input-field"></div>
            </div>
            <div class="input-wrap">
                <label>Title</label>
                <div class="cf"><input type="text" name="title" value="<?php if (!empty($edit_data)) {
    echo $edit_data['singer_title'];
} ?>" class="input-field"></div>
            </div>
            <div class="input-wrap">
                <label>description</label>
                <div class="cf"><textarea class="description" name="description" rows="10" cols="40"><?php if (!empty($edit_data)) {
    echo $edit_data['singer_desc'];
} ?></textarea></div>
            </div>
            <div class="input-wrap">
                <label>keywords</label>
                <div class="cf"><input type="text" name="keyword" value="<?php if (!empty($edit_data)) {
    echo $edit_data['singer_keywords'];
} ?>" class="input-field"></div>
            </div>

    <?php if (!empty($edit_data)) { ?>
                <input type="submit" name="submit" class="button" value="Update">
                <a href="<?php echo base_url() . "backend/singer/status?id=" . $edit_data['id']; ?>" class="button delete">delete</a>
    <?php } ?>
        </div>
    </div>
<?php if (empty($edit_data)) { ?>
    ...
    $(document).ready(function () {
        ...
        video_table = $('#video-example').DataTable({
            ...
            "ajax": {
                ...
                "type": "POST",
                "data": {
                    singer_id: "<?php echo $edit_data['id']; ?>"
