<div class="ctbody-content">
    ...
    <?php if (!empty($edit_data)) { ?>
        <div class="addcategory-total cf">
            ...
            if (!empty($edit_data)) {
                echo base_url()."personality/".str_replace(" ", "-", str_replace(["(", ")"], "", $edit_data['personality_name']));
            }
            ?>" class="view-website">View in Website</a>
        </div>
    <?php } ?>
    <div class="user-profile cf">
        <?php echo form_open_multipart((!empty($edit_data)) ? base_url('backend/personality/update') : base_url('backend/personality/save'), array('method' => 'POST')); ?>
        <?php if (!empty($edit_data)) { ?>
            <input type="hidden" name="id" value="<?php echo $edit_data['id']; ?>">
        <?php } ?>
        <div class="image-name">
            <img src="<?php echo (!empty($edit_data) && !empty($edit_data['personality_img'])) ? base_url() . "images/personality/" . $edit_data['personality_img'] : base_url() . 'images/no-user.svg'; ?>" alt=""  id="img">
        ...
        <div class="info-block">
            <div class="input-wrap">
                ...
                <div class="cf"><input type="text" name="name" value="<?php
                    if (!empty($edit_data)) {
                        echo $edit_data['personality_name'];
                    }
            ...
            <div class="input-wrap">
                ...
                <div class="cf"><input type="text" name="title" value="<?php
                    if (!empty($edit_data)) {
                        echo $edit_data['personality_title'];
                    }
            ...
            <div class="input-wrap">
                ...
                <div class="cf"><textarea class="description" name="description" rows="10" cols="40"><?php
                        if (!empty($edit_data)) {
                            echo $edit_data['personality_desc'];
                        }
                        ?></textarea></div>
            </div>
            <div class="input-wrap">
                <div class="category-wrap cf">
                    ...
                    <div class="language"><span>Cast</span><input type="checkbox" value="1" name="is_cast" class="checkbox" <?php echo (!empty($edit_data) && $edit_data['is_cast'] == 1)? "checked" : ""; ?>></div>
                    <div class="language"><span>Singer</span><input type="checkbox" value="1" name="is_singer" class="checkbox" <?php echo (!empty($edit_data) && $edit_data['is_singer'] == 1)? "checked" : ""; ?>></div>
                    <div class="language"><span>Director</span><input type="checkbox" value="1" name="is_director" class="checkbox" <?php echo (!empty($edit_data) && $edit_data['is_director'] == 1)? "checked" : ""; ?>></div>
                    <div class="language"><span>Music Director</span><input type="checkbox" value="1" name="is_music_director" class="checkbox" <?php echo (!empty($edit_data) && $edit_data['is_music_director'] == 1)? "checked" : ""; ?>></div>
                </div>
            </div>
            ...
            <?php if(!empty($edit_data)) { ?>
                ...
                <div class="input-wrap">
                <div class="category-wrap cf">
                    <label>Seo Role Played</label>                    
                    <div class="language"><span>Cast</span><input type="radio" value="is_cast" name="for_seo" class="" <?php echo (!empty($edit_data) && $edit_data['for_seo'] == 'is_cast')? 'checked="checked"' : ""; ?>></div>
                    <div class="language"><span>Singer</span><input type="radio" value="is_singer" name="for_seo" class="" <?php echo (!empty($edit_data) && $edit_data['for_seo'] == 'is_singer')? 'checked="checked"' : ""; ?>></div>
                    <div class="language"><span>Director</span><input type="radio" value="is_director" name="for_seo" class="" <?php echo (!empty($edit_data) && $edit_data['for_seo'] == 'is_director')? 'checked="checked"' : ""; ?>></div>
                    <div class="language"><span>Music Director</span><input type="radio" value="is_music_director" name="for_seo" class="" <?php echo (!empty($edit_data) && $edit_data['for_seo'] == 'is_music_director')? 'checked="checked"' : ""; ?>></div>
            ...
            <div class="input-wrap">
                ...
                <div class="cf"><input type="text" name="keyword" value="<?php
                    if (!empty($edit_data)) {
                        echo $edit_data['personality_keywords'];
                    }
                    ?>" class="input-field"></div>
            </div>
            <?php if (!empty($edit_data)) { ?>
                <input type="submit" name="submit" class="button" value="Update">
                <a href="<?php echo base_url() . "backend/personality/status?id=" . $edit_data['id']; ?>" class="button delete">delete</a>
            <?php } ?>
        </div>
    </div>
    <?php if (empty($edit_data)) { ?>
    ...
    $(document).ready(function () {
        ...
        video_table = $('#video-example').DataTable({
            ...
            "ajax": {
                ...
                "type": "POST",
                "data": {
                    personality_id: "<?php echo $edit_data['id']; ?>"
                },
        ...
        $('.button-search').on('click', function (e) {
            ...
            poster_table.columns(0).search(year).columns(1).search(month).columns(2).search(search_keyword).columns(3).search($('#start_date').val()).columns(4).search($('#end_date').val()).draw();
