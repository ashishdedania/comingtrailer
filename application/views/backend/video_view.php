...
' . $msg[1] . '!
    ...
    <?php if (!empty($edit_data)) { ?>
        <div class="addcategory-total cf">
            <a target="_blank" href="<?php if (!empty($edit_data)) {
        echo $edit_data['final_url'];
    } ?>" class="view-website">View in Website</a>
        </div>
        <?php } ?>
    <div class="videoposter-info">
        <?php echo form_open_multipart((!empty($edit_data)) ? base_url('backend/video/update') : base_url('backend/video/save'), array('method' => 'POST')); ?>
        <?php if (!empty($edit_data)) { ?>
            <input type="hidden" name="id" value="<?php echo $edit_data['id']; ?>">
<?php } ?>
        <input type="hidden" id="videoid" name="videoid" value="">
        <input type="hidden" id="videothumburl" name="videothumburl" value="">
        <div class="input-wrap">
            <label>Url</label>
            <div class="cf">
                <input type="url" required name="videourl" id="videourl" value="<?php if (!empty($edit_data)) {
    echo $edit_data['video_url'];
} ?>" class="input-field">
        ...
        <div class="date-category cf">
            <div class="row">
                <div class="col-md-6">
                    <div class="input-wrap">
                        <label>Date</label>
                        <input type="text" required value="<?php if (!empty($edit_data) && date("d-m-Y", strtotime($edit_data['rel_date'])) != "01-01-1970") {
    echo $edit_data['rel_date'];
} ?>" id="date" name="date"  placeholder="DD-MM-YYYY" class="input-field datepic">
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="input-wrap">
                        <label>Main category</label>
                        <select class="select"  name="cat_id" id="cat_id" required="" style="width: 100%"  id="cat_id">
                            <option selected="selected" disabled="disabled" value="">Select</option>
<?php if (!empty($main_category)) {
    foreach ($main_category as $row) { ?>
                                    <option <?php if (!empty($edit_data) && $edit_data['cat_id'] == $row['id']) {
            echo "selected";
                ...
                <div class="col-md-3">
                    <div class="input-wrap">
                        ...
                        <select class="select" name="subcat_id" required="" style="width: 100%"  id="subcat_id">
                            <option selected="selected" disabled="disabled" value="">Select</option>
<?php if (!empty($category)) {
    foreach ($category as $row) { ?>
                                    <option <?php if (!empty($edit_data) && $edit_data['subcat_id'] == $row['id']) {
            echo "selected";
        } ?> value="<?php echo $row['id']; ?>"><?php echo $row['subcat_name']; ?></option>
    <?php }
} ?>
                        </select>
                    </div>
    ...
    <div class="videoposter-info">
        <div class="input-wrap">
            <label>Name</label>
            <div class="cf">
                <input type="text" required value="<?php if (!empty($edit_data)) {
    echo $edit_data['video_name'];
} ?>" id="name" name="name" class="input-field full-field">
            </div>
        </div>
        <div class="input-wrap">
            <label>description</label>
            <div class="cf">
                <textarea name="description" id="description" class="description" rows="10" cols="40"><?php if (!empty($edit_data)) {
    echo $edit_data['video_desc'];
} ?></textarea>
            </div>
        </div>
        <div class="input-wrap">
            ...
            <div class="cf">
                <input type="text" value="<?php if (!empty($edit_data)) {
    echo $edit_data['video_keywords'];
} ?>" name="keywords" id="keywords" class="input-field full-field">
            </div>
        </div>
        <div class="input-wrap">
            <label>Movie</label>
            <div class="cf">
                <select style="width: 100%" id="movie"  name="movie_id" class="js-data-example-ajax">
                <?php if (!empty($edit_data)) {
                    echo "<option value='" . $edit_data['movie_id'] . "'>" . $edit_data['movie_name'] . "</option>";
                } ?>
                </select>
            </div>
        ...
        <div class="input-wrap">
            ...
            <div class="cf">
                <select style="width: 100%" id="cast" name="cast[]" multiple class="personality js-data-example-ajax">
                    <?php if (!empty($cast_list) && !empty($cast_list)) {
...
<?php if (!empty($singers_list) && !empty($edit_data)) {
...
<?php if (!empty($music_list) && !empty($edit_data)) {
...
<?php if (!empty($director_list) && !empty($edit_data)) {
...
<?php if (!empty($edit_data)) { ?>
                ...
                <a href="<?php echo base_url() . "backend/video/status?id=" . $edit_data['id']; ?>" class="button-delete btn">delete</a>
<?php } else { ?>
...
<script type="text/javascript">
            ...
            if (typeof (fileUpload.files) != "undefined") {
                ...
                reader.onload = function (e) {
                    ...
                    image.onload = function () {
                        ...
                        $('#videoimg') .attr('src', e.target.result);
                        document.getElementById("videothumburl").value = '';
                        return true;
                    };
    ...
    function getVideoThumb() {
        ...
        if (videoid != null) {
            ...
            document.getElementById("videothumburl").value = "https:"+finalurl;
    ...
    $('#cat_id').on('change', function (e) {
        ...
        if (val == 1) {
            ...
            $("#movie").prop("required", true);
        }
        else {
            $("#movie").prop("required", false);
        }
