<div class="ctbody-content">
    ...
    <?php if (!empty($edit_data)) { ?>
        <div class="addcategory-total cf">
            <p class="total-number"><span id="trailer_total">0</span> Trailers</p> <p class="total-number"><span id="song_total">0</span> Songs</p>
    ...
    <div class="user-profile cf">
        <?php echo form_open_multipart((!empty($edit_data)) ? base_url('backend/channel/update') : base_url('backend/channel/save'), array('method' => 'POST')); ?>
        <?php if (!empty($edit_data)) { ?>
            <input type="hidden" name="id" value="<?php echo $edit_data['id']; ?>">
<?php } ?>
        <div class="image-name">
            <img src="<?php echo (!empty($edit_data) && !empty($edit_data['rel_by_img'])) ? base_url() . "images/channel/" . $edit_data['rel_by_img'] : base_url() . 'images/no-movie.svg'; ?>" alt=""  id="img">
...
?>" class="input-field"></div>
            ...
            <div class="input-wrap">
                ...
                <div class="cf"><input type="text" name="keyword" value="<?php
                    if (!empty($edit_data)) {
                        echo $edit_data['rel_by_keywords'];
                    }
                    ?>" class="input-field"></div>
            </div>
    ...
    <?php if (!empty($edit_data)) { ?>
                ...
                <a href="<?php echo base_url() . "backend/channel/status?id=" . $edit_data['id']; ?>" class="button delete">delete</a>
    <?php } ?>
        </div>
    </div>
<?php if (empty($edit_data)) { ?>
    ...
    $(document).ready(function () {
        ...
        video_table = $('#video-example').DataTable({
            ...
            "ajax": {
                ...
                "type": "POST",
                "data": {
                    channel_id: "<?php echo $edit_data['id']; ?>"
