<div id="ctwpcontent">
    ...
    <li><a href="<?php echo base_url('AdmViewActor');?>" class="<?php if($is_delete == 'no'){ echo 'active';}?>">All</a></li>
        
    <li><a href="<?php echo base_url($this->router->fetch_class());?>/deleted/cast" class="<?php if($is_delete == 'yes'){ echo 'active';}?>">DELETE</a></li>    
</ul>
			...
			<p class="total-number">
                             <?php 
                                        foreach ($actors as $value) {
                                            echo ''.$value['count'];?> Actors
                        <?php
                        break;
                                        }
                                    ?>
                        </p>
                        <a href="<?php echo base_url();?>AdmAddActor" class="button icon-add">Add new Actor</a>
		</div>
    <div class="search-bar cf">
			
                <form method="post" name="seo_form" action="<?php echo base_url('AdmViewActor'); ?>">    
			...
			<div>
				<ul class="list">
                                    ...
                                    <li>
                                        ...
                                        if($is_delete == 'yes'){
                                            ?>
                                        <a href="<?php echo base_url();?>AdmViewActor/editActor/<?php echo ''.$value['cast_id'];?>" name="names">
                                            
                                        <?php echo ''.$value['cast_name'];?>
                                        </a>
                                        <div class="restore-delete">
                                            <a href="<?php echo base_url();?>AdmAddActor/deleteStatus/cast/<?php echo ''.$value['cast_id'];?>/0" class="icon-restore" onclick="return confirm('Are you sure to reactive this actor?')"></a>
                                            <a href="<?php echo base_url('AdmAddActor/delete/cast/'.$value['cast_id'].'/'.$value['seo_url_id']); ?>" class="icon-delete" onclick="return confirm('Are you sure to parmenatlly delete this actor?')"></a>
                                        </div>
                                        <?php
                                        }else{
                                        ?>
