<div id="ctwpcontent">
    ...
    <li><a href="<?php echo base_url('AdmViewDirector');?>" class="<?php if($is_delete == 'no'){ echo 'active';}?>">All</a></li>
			...
			<p class="total-number">
                             <?php 
                                        foreach ($actors as $value) {
                                            echo ''.$value['count'];?> Director
                        <?php
                        break;
                                        }
                                    ?>
                        </p>
                        <a href="<?php echo base_url();?>AdmAddDirector" class="button icon-add">Add New Director</a>
		</div>
    <div class="search-bar cf">
        <form method="post" name="seo_form" action="<?php echo base_url('AdmViewDirector'); ?>">    
			...
			<div>
				<ul class="list">
                                    ...
                                    <li>
                                        <?php if($is_delete == 'no'){
                                            ?>
                                        <a href="<?php echo base_url();?>AdmViewDirector/edit/<?php echo ''.$value['director_id'];?>" name="names"><?php echo ''.$value['director_name'];?></a>
                                        <?php
                                        }else{
                                            ?>
                                        <a href="<?php echo base_url();?>AdmViewDirector/edit/director/<?php echo ''.$value['director_id'];?>" name="names"><?php echo ''.$value['director_name'];?></a>
                                        <div class="restore-delete">
                                            <a href="<?php echo base_url();?>AdmAddDirector/deleteStatus/director/<?php echo ''.$value['director_id'];?>/0" class="icon-restore" onclick="return confirm('Are you sure to reactive this director?')"></a>
                                            <a href="<?php echo base_url('AdmAddDirector/delete/director/'.$value['director_id'].'/'.$value['seo_url_id']); ?>" class="icon-delete" onclick="return confirm('Are you sure to parmenatlly delete this director?')"></a>
                                        </div>
                                        <?php
                                        }
                                        }
